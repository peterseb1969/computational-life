...
Every host found in the runs without head initialisation, and the winner of the mini's first heads run (`,[[>..}]-[+.` in `-i`), copies from head0 to head1 with `.`, and the loop tests the *source*: `[..{>]`, `[>..}]`. The copy length is fixed by the host's own bytes, so it writes over whatever the partner contains and its output does not depend on the target. This spike's loop copies from head1 to head0 with `,` and tests the destination, so it is selective about targets, blocked by its own zeros, and unable to overwrite its own failures. A source-tested copier is robust to a saturated soup; a destination-tested one poisons its soup and dies of the poison. The heads variant makes the destination-tested kind easier to build, because the two head bytes can put head0 at the far end of the tape without any head-moving instructions, and this one needed only four instructions in a loop.
